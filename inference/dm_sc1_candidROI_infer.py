...
def run(img_folder, img_height=1024, img_scale=4095, 
    ...
    def pred_img_list(img_list):
        roi_generator = img_gen.flow_from_candid_roi(
            ...
            blob_th_step=blob_th_step, roi_clf=None, seed=random_seed)
        roi_dat,_ = roi_generator.next()
    ...
    for subj, exidx, exam in exam_list:
        try:
            predL_arr = pred_img_list(exam['L']['img'])
        except KeyError:
            predL_arr = np.array([.0])
        try:
            predR_arr = pred_img_list(exam['R']['img'])
        except KeyError:
            predR_arr = np.array([.0])
        predL_arr.sort()
        predR_arr.sort()
        predL = predL_arr[-nb_top_avg:].mean()
        predR = predR_arr[-nb_top_avg:].mean()
